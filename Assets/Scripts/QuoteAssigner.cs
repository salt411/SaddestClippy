...
public class QuoteAssigner : MonoBehaviour
{
    ...
    public TextMeshPro textMesh; // Assign in the inspector, reference to the TextMesh component

    void Start()
    {
        AssignRandomText();

        // Assume this script is attached to the cube
        GameObject cube = this.gameObject;
    }

    void AssignRandomText()
    {
        string randomText = texts[UnityEngine.Random.Range(0, texts.Length)]; // Select a random text
        textMesh.text = randomText; // Assign it to the TextMesh
    }
}
